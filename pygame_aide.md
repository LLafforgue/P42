...
## Fonction Bit Block Transfer :

la fonction blit copier un bloc de pixels d'une image vers une autre.
```python
surface_destination.blit(surface_source, position)
```
...
while running:
    ...
    # --- Dessinez TOUJOURS sur logical_surface ---
    logical_surface.fill((30, 30, 30))
    pygame.draw.circle(logical_surface, (255, 100, 0), (400, 300), 80)

    # --- Mise à l'échelle vers la taille actuelle de la fenêtre ---
    win_w, win_h = window.get_size()

    # Calcul du ratio en conservant les proportions
    scale = min(win_w / LOGICAL_W, win_h / LOGICAL_H)
    scaled_w = int(LOGICAL_W * scale)
    scaled_h = int(LOGICAL_H * scale)

    scaled_surface = pygame.transform.scale(logical_surface, (scaled_w, scaled_h))

    # Centrage dans la fenêtre (bandes noires si proportions différentes)
    x = (win_w - scaled_w) // 2
    y = (win_h - scaled_h) // 2

    window.fill((0, 0, 0))  # fond noir pour les bandes
